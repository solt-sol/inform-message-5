Последняя версия кода на момент 25.10.22

В коде есть коментарии, а также при его работе могут выводиться подсказывающие принты
```
...
async def once_done(sink: discord.sinks, channel: discord.TextChannel, *args): # функция куда идет аудиофайл, а также его последующая обработка
    ...
    text = "Сними коментарий через 5 строчек вниз"
